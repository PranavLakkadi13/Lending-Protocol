...
contract LendingPoolCore {
    ...
    event SimpleWithdraw(address indexed depositor, uint256 indexed amount, uint256 indexed depositCounter);
    ...
    function withdrawLiquidity(address user, uint256 amount, uint256 depositId) public onlyRouter {
        require(msg.sender == i_Router, "Only router can call this function");
        ...
        if (amount == deposit.trackedDeposits[depositId].amount) {
            ...
            uint256 amountWIthInterest = _calculateTheAmountOfTokensToReturn(
                amount, deposit.trackedDeposits[depositId].timeOfDeposit, totalBalance
            );
    ...
    function withdrawTotalAmount(address user) public onlyRouter {
        ...
        for (uint256 i = 0; i < s_userDeposits[user].depositCounter; i++) {
    ...
    function _calculateTheAmountOfTokensToReturn(uint256 amount, uint256 timeOfDeposit, uint256 BalanceOfContract)
        internal
        view
        returns (uint256 amountWithInterest)
    {
    ...
    // will look into this
    function getBorrowableAmountBasedOnUSDAmount(address user, uint256 collateralInUSD)
        external
        view
    ...
    /// @param amount amount of tokens
    function FlashLoan(address receiver, uint256 amount) external onlyRouter {
